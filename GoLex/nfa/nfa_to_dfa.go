...
func (n *NfaDfaConverter) printRowMap() {
	//bug here len(n.dtran) -> len(n.rowMap)
	fmt.Fprintf(n.fp, "%s  %s  Yy_rmap[%d]=\n{\n    ", SCLASS, TYPE, len(n.rowMap))
	n.printMap(n.rowMap)
}
...
func (n *NfaDfaConverter) addDfaState(epsilonResult *EpsilonResult) int {
	...
	//if n.dstates[nextState].isAccepted {
	//	fmt.Printf("accept found with state %d, with string: %s\n", nextState, n.dstates[nextState].acceptString)
	//}
	n.dstates[nextState].LineNo = epsilonResult.LineNo

	n.dstates[nextState].anchor = epsilonResult.anchor
	//这里使得 state 的数值与 DFA 对象在数组中的下标一致
...
func (n *NfaDfaConverter) printMove(set []*NFA) {
	count := len(set)
	for i := 0; i < count; i++ {
		fmt.Printf("\nmove state: %d ,", set[i].state)
	}
	fmt.Printf("\n")
}
...
func (n *NfaDfaConverter) PrintDfaTransition() {
	for i := 0; i < n.nstates; i++ {
		...
		fmt.Printf("current state %d,is accepting state: %t\n", i, n.dstates[i].isAccepted)
		if n.dstates[i].isAccepted {
			fmt.Printf("accept string is %s\n", n.dstates[i].acceptString)
		}
...
func (n *NfaDfaConverter) initGroups() {
	...
	for i := 0; i < n.nstates; i++ {
		if n.dstates[i].isAccepted {
			/*
				所有拥有相同接收态字符串的节点处于同一分区
				遍历前面节点，如果遇到接收态节点，判断其接收字符串是否与当前节点接收字符串相同，
				如果相同，取出前面节点的分区，将当前节点加入其分区
			*/
			foundGroup := false
			group := n.numGroups
			for j := i - 1; j >= 0; j-- {
				if n.dstates[j].isAccepted && n.dstates[j].acceptString == n.dstates[i].acceptString {
					group = n.inGroups[j]
					foundGroup = true
					break
				}
			...
			n.groups[group] = append(n.groups[group], n.dstates[i].state)
			//记录状态点对应的分区
			n.inGroups[n.dstates[i].state] = group
			if !foundGroup {
				n.numGroups += 1
			}
		} else {
...
func (n *NfaDfaConverter) fixTran() {
	...
	*/
	...
	//最小化 DFA 状态机后，需要更新 dstates 数组，每个分区对应一个新的 dfa 节点
	newDstates := make([]DFA, DFA_MAX)
	n.nstates = n.numGroups
	for i := 0; i < n.numGroups; i++ {
		//从当前分区取出一个节点即可
		state := n.groups[i][0]
		//设置新的 DFA 节点信息，这里我们只需要知道其是否为接收节点，以及对应接收字符串
		oldDFA := n.dstates[state]
		newDstates[i].state = i
		newDstates[i].isAccepted = oldDFA.isAccepted
		newDstates[i].acceptString = oldDFA.acceptString
		newDstates[i].mark = true
