...
func (l *LexReader) Tail() {
	//将 input.lex 在最后一个 %% 后面的内容拷贝到 lex.yy.c
...
func (l *LexReader) GetExpr() string {
	...
	*/
	...
	for haveLine {
		...
		if currentLine[len(currentLine)-1] == uint8('|') {
			/*
				当正则表达式的末尾是 | 时，它表明下一行将会和当前行的内容结合在作为同一个正则表达式的一部分，例如
				is|
				am|
				go
				则对应的正则表达式应该是 is|am|go
			*/
